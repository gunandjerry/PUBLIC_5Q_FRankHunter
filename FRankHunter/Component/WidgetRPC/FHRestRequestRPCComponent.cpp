// Copyright F Rank Hunter. All Rights Reserved.


#include "Component/WidgetRPC/FHRestRequestRPCComponent.h"
#include "Lobby/FHLobbyGameMode.h"

// Sets default values for this component's properties
UFHRestRequestRPCComponent::UFHRestRequestRPCComponent()
{
	// Set this component to be initialized when the game starts, and to be ticked every frame.  You can turn these features
	// off to improve performance if you don't need them.
	PrimaryComponentTick.bCanEverTick = true;

	// ...
}

void UFHRestRequestRPCComponent::AnswerRest_Implementation(bool Result)
{
	APlayerController* Owner = GetOwner<APlayerController>();
	check(Owner);

	AFHLobbyGameMode* GameMode = Cast<AFHLobbyGameMode>(GetWorld()->GetAuthGameMode());
	if (GameMode)
	{
		GameMode->AnswerRest(Result, Owner);
	}
}

